...
def start_ai():
    ...
    while running:
        ...
        graph_index.append(len(graph_data))

        cv2.imshow("SMART CROWD AI DASHBOARD", frame)
...
def live_graph(i):
    plt.cla()
    plt.plot(graph_index, graph_data, marker='o')
    plt.title("LIVE CROWD DASHBOARD GRAPH")
    plt.xlabel("Time")
...
def start_graph():
    fig = plt.figure()
    ani = animation.FuncAnimation(fig, live_graph, interval=1000)
    plt.show()

# ---------------- GUI DASHBOARD ---------------- #
root = tk.Tk()
root.title("SMART CROWD AI DASHBOARD")
root.geometry("550x500")

title = tk.Label(root, text="SMART CROWD AI DASHBOARD",
                 font=("Arial", 15, "bold"))
title.pack(pady=10)
...
# -------- GRAPH -------- #
tk.Button(root, text="START LIVE GRAPH",
          bg="purple", fg="white",
          command=lambda: threading.Thread(target=start_graph).start()
).pack(pady=10)
